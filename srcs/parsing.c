...
/*   Updated: 2024/07/09 14:52:55 by aelkheta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_command	*parser_command(t_command *_tokens_list)
{
	...
	{
		list_command = new_node(_tokens_list->type,
				ft_strdup(_tokens_list->value));
		...
		{
			i = 0;
			list_command->args = malloc((get_args_size(_tokens_list) + 2)
					* sizeof(char *));
			while (_tokens_list != NULL && _tokens_list->type != PIPE)
			{
				...
				_tokens_list = free_node(_tokens_list);
				while (_tokens_list != NULL && (_tokens_list->type == RED_IN
						|| _tokens_list->type == RED_OUT
						|| _tokens_list->type == APP
						|| _tokens_list->type == HER_DOC))
				{
					_tokens_list = redirect_list(&head, list_command,
							_tokens_list, &rdrct_head);
					if (data->syntax_error)
						return (NULL);
				}
			}
		}
		else if (_tokens_list->type == RED_IN || _tokens_list->type == RED_OUT
			|| _tokens_list->type == APP || _tokens_list->type == HER_DOC)
		{
			_tokens_list = free_node(_tokens_list);
			if (!_tokens_list || _tokens_list->type != TOKEN)
			{
